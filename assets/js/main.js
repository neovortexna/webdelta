...
/**
 * ============================================================
 * TOGGLE SIDEBAR (MENU KIRI) — berlaku di SEMUA halaman
 * ------------------------------------------------------------
 * Sebelumnya panel menu kiri hanya bisa dibuka/tutup di halaman
 * Notifikasi Expired (mode tampilan TV). Sekarang tombol hamburger
 * di header bisa membuka/menutup sidebar di SEMUA halaman & semua
 * ukuran layar:
 *
 *   - Layar besar (laptop/PC/TV, lebar >960px): sidebar DILIPAT
 *     (disembunyikan ke kiri) dan konten utama melebar mengisi
 *     ruang kosong. Preferensi "dilipat" ini diingat lewat
 *     localStorage supaya tetap terasa saat halaman dibuka lagi.
 *
 *   - Layar kecil (tablet/HP, lebar <=960px): sidebar tersembunyi
 *     secara default lalu tampil sebagai panel yang ditarik masuk
 *     dari kiri (overlay) begitu tombol hamburger ditekan. Panel
 *     ini bisa ditutup lagi lewat: tombol (X) di dalam panel, klik
 *     area gelap di luar panel, tombol Esc, atau memilih salah
 *     satu menu di dalamnya.
 * ============================================================ */
$(function () {
    const $toggle  = $('#btn-sidebar-toggle');
    const $close   = $('#btn-sidebar-close');
    const $overlay = $('#sidebar-overlay');
    if (!$toggle.length) return; // halaman ini tidak punya sidebar (mis. login.php)

    const MEDIA_DESKTOP = '(min-width: 961px)';
    const STORAGE_KEY   = 'simas_sidebar_collapsed';
    ...
    // Terapkan preferensi "dilipat" yang tersimpan (hanya relevan di layar besar)
    ...
    $toggle.on('click', function () {
        if (isDesktop()) {
            ...
            if ($('body').hasClass('sidebar-open')) tutupSidebarMobile(); else bukaSidebarMobile();
        }
    });
    $close.on('click', tutupSidebarMobile);
    $overlay.on('click', tutupSidebarMobile);
    $(document).on('keyup', function (e) {
        if (e.key === 'Escape') tutupSidebarMobile();
    });

    // Setelah memilih salah satu menu di panel overlay (tablet/HP), panel
    // ditutup otomatis supaya halaman baru langsung terlihat penuh.
    $('#app-sidebar .sidenav__link').on('click', function () {
        if (!isDesktop()) tutupSidebarMobile();
    });

    // Saat ukuran layar berubah (mis. rotasi tablet, jendela di-resize),
    // bersihkan status yang sudah tidak relevan supaya tidak "nyangkut".
    let resizeTimer;
